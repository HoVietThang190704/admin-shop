...
export default function EditProductPage() {
    ...
    const loadData = async () => {
      ...
      try {
        const [productRes, categoriesRes] = await Promise.all([
          ProductService.getInstance().getProductById(productId),
        ...
        if (productRes.success && productRes.data) {
          setFormData({
            ...
            imageUrl: productRes.data.images?.[0] || '',
          });
        } else {
          setError(productRes.message || 'Không tải được sản phẩm');
        }
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await ProductService.getInstance().updateProduct(productId, {
        title: formData.title.trim(),
        price: Number(formData.price),
        description: formData.description,
        category: formData.category,
        ...(formData.imageUrl.trim() ? { images: [formData.imageUrl.trim()] } : {}),
      });
